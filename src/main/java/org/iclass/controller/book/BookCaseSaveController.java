...
public class BookCaseSaveController implements Controller {
	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MultipartRequest multiRequset = new MultipartRequest(request,
				path,
				maxSize,
				"UTF-8",
		...
		String bcode = multiRequset.getParameter("bcode");
		String title = multiRequset.getParameter("title");
		String temp = multiRequset.getParameter("writer");
		String publisher = multiRequset.getParameter("publisher");
		String pubdate = multiRequset.getParameter("pubdate");
		String saleuser = multiRequset.getParameter("saleuser");
		int price = Integer.parseInt(multiRequset.getParameter("price"));
		// 서버의 파일 시스템 D:\\class231228\\upload 경로에서 파일이 생겼는지 확인하기
		
		//서버로 업로드된 파일의 파일명 저장
		String cover = multiRequset.getFilesystemName("cover");
